...
func TestDbHandlerConnectError(t *testing.T) {
	for _, test := range []struct{ drv, url string }{{drv: "postgres", url: "host=127.0.0.1 port=45 dbname=delayed_test user=delayedtest password=123456 sslmode=disable"},
...
func TestDbHandlerAuthError(t *testing.T) {
	handler, e := newDbHandler(map[string]interface{}{}, map[string]interface{}{"script": "select 2", "drv": "postgres", "url": "host=127.0.0.1 dbname=delayed_test user=ssdelayedtest password=ww123456 sslmode=disable"})
	if nil != e {
		t.Error(e)
		return
	}
